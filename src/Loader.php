...
class Loader
{
    ...
    public static function import($class, $baseUrl = '', $ext = EXT)
    {
        ...
        if (empty($baseUrl)) {
            ...
            if (isset(self::$prefixDirsPsr4[$name . '\\'])) {
                ...
                // 项目根目录下查找：extend/<name> 或 app/<name>
                $root     = dirname(__DIR__) . DS;
                $extendDp = $root . 'extend' . DS;
                $appDp    = $root . 'app' . DS;
                if (defined('EXTEND_PATH')) {
                    $extendDp = EXTEND_PATH;
                }
                if (defined('APP_PATH')) {
                    $appDp = APP_PATH;
                }
                if (is_dir($extendDp . $name)) {
                    $baseUrl = $extendDp . $name . DS;
                } else {
                    $baseUrl = $appDp . $name . DS;
                }
            }
    ...
    {
        if (false !== ($len = strrpos($url, '\\')) || false !== ($len = strrpos($url, '/'))) {
            $action = substr($url, $len + 1);
            $module = substr($url, 0, $len);
        } else {
            $action = $url;
            $module = Request::instance()->controller();
        }

        // 修复 linux 上 pathinfo('\a\b\c')['dirname'] 为 . 的问题（win 和 Linux 表现不一致）
        // $info = pathinfo($url);
