...
class Validate
{
    ...
    protected static $typeMsg = [
        ...
        'image'       => ':attribute不是有效的图像',
        'alpha'       => ':attribute只能是字母',
        'alphaNum'    => ':attribute只能是字母和数字',
        'alphaDash'   => ':attribute只能是字母、数字、下划线及破折号',
        'activeUrl'   => ':attribute不是有效的域名或IP',
        'chs'         => ':attribute只能是汉字',
        'chsAlpha'    => ':attribute只能是汉字、字母',
        'chsAlphaNum' => ':attribute只能是汉字、字母、数字',
        'chsDash'     => ':attribute只能是汉字、字母、数字、下划线及破折号',
        'url'         => ':attribute不是有效的URL地址',
        'ip'          => ':attribute不是有效的IP地址',
        'dateFormat'  => ':attribute必须符合日期格式 :rule',
        'in'          => ':attribute必须在 :rule 范围内',
        'notIn'       => ':attribute不能在 :rule 范围内',
        'between'     => ':attribute必须在 :1 - :2 之间',
        'notBetween'  => ':attribute不能在 :1 - :2 之间',
        'length'      => ':attribute长度必须为 :rule',
        'max'         => ':attribute最大长度为 :rule',
        'min'         => ':attribute最小长度为 :rule',
        'after'       => ':attribute不能小于 :rule',
        'before'      => ':attribute不能超过 :rule',
        'afterWith'   => ':attribute不能小于 :rule',
        'beforeWith'  => ':attribute不能超过 :rule',
        'expire'      => ':attribute不在有效期内 :rule',
        'allowIp'     => '不允许的IP访问',
        'denyIp'      => '禁止的IP访问',
        'confirm'     => ':attribute和 :2 不一致',
        'different'   => ':attribute和 :2 不能相同',
    ...
    protected function checkItem($field, $value, $rules, $data, $title = '', $msg = [])
    {
        ...
        foreach ($rules as $key => $rule) {
            if ($rule instanceof \Closure) {
                ...
                // 如果不是 require 系列规则，且值为空则跳过验证
                // r_if / r_with / r_with_all / r_without / r_without_all 是 yf 风格的 require 系列规则，
                // 即使值为空也必须触发（用于条件必填场景）
                $isRequireLike = 0 === strpos($info, 'require')
                    || 0 === strpos($info, 'r_');
                if ($isRequireLike || (!is_null($value) && '' !== $value)) {
                    // 验证类型
                    $callback = isset(self::$type[$type]) ? self::$type[$type] : [$this, $type];
                    // 验证数据
            ...
            if (false === $result) {
                ...
                if (isset($msg[$i])) {
                    $message = $msg[$i];
                    // think-orm: 已剥离多语言包；{%xxx%} 包装被直接剥离为字面字符串
    ...
    /**
     * 使用行为类验证
     * 行为钩子已移除：本包不支持 Hook 机制。behavior 规则永远视为通过。
     * 如需自定义行为验证，请覆盖本方法或直接在 Validate 子类中实现规则。
     * @access protected
     * @param mixed     $value  字段值
     * @param mixed     $rule  验证规则
     * @param array     $data  数据
