...
class Query
{
     ...
     * @return string
     */
    public function getLastSql($isReal = true)
    {
        return $this->connection->getLastSql($isReal);
    }
    ...
    public function setInc($field, $step = 1, $lazyTime = 0)
    {
        ...
        // think-orm：永远实时写入 DB，忽略 $lazyTime（不依赖 Cache 累积机制）
     ...
     * 延时更新检查（已废弃）
     *
     * think-orm：永远实时写入 DB，不再支持延迟累积更新机制。
     * 保留方法签名以维持 API 兼容，但 setInc/setDec 已不再调用本方法。
     *
     * @access protected
    ...
    public function field($field, $except = false, $tableName = '', $prefix = '', $alias = '')
    {
        ...
        $_field1 = array_unique($_field1);

        $field = array_merge($_field1, $_field2);
    ...
    public function getTableInfo($tableName = '', $fetch = '')
    {
        ...
        if (!isset(self::$info[$db . '.' . $guid])) {
            ...
            // 读取缓存
            if ($this->getConfig('use_schema') && is_file(RUNTIME_PATH . 'schema/' . $schema . '.php')) {
    ...
    public function with($with)
    {
        ...
        $with = array_merge($_field1, $_field2);
        ...
        foreach ($with as $key => $relation) {
            ...
            if (is_numeric($key)) {
                if (isset($this->options['with'])) {
                    $_i = array_search($relation, $this->options['with']);
                    if ($_i !== false) {
                        unset($this->options['with'][$_i]);
                        continue;
                    }
                }
            }
